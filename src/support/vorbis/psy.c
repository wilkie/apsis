...
static float ***setup_tone_curves(float curveatt_dB[P_BANDS],float binHz,int n,
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    for(j=0;j<P_LEVELS;j++){
      attenuate_curve(workc[i][j],curveatt_dB[i]+(float)(100.-(j<2?2:j)*10.-P_LEVEL_0));
...
static void flag_lossless(int limit, float prepoint, float postpoint, float *mdct,
  ...
  for(j=0;j<jn;j++){
    ...
    float r = (float)(fabs(mdct[j])/floor[j]);
    if(r<point)
      flag[j]=0;
    else
      flag[j]=1;
        ...
        for(j=0;j<jn;j++){
          ...
          if(j<sliding_lowpass-i){
            if(fM[j] || fA[j]){
              ...
              reM[j] = (float)(fabs(reM[j])+fabs(reA[j]));
              qeM[j] = qeM[j]+qeA[j];
              fM[j]=fA[j]=1;
              ...
              if(j<limit-i){
                ...
                */
                ...
                /* elliptical */
                if(reM[j]+reA[j]<0){
                  reM[j] = - (qeM[j] = (float)(fabs(reM[j])+fabs(reA[j])));
                }else{
                  reM[j] =   (qeM[j] = (float)(fabs(reM[j])+fabs(reA[j])));
                }
#endif
