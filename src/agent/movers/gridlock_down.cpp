...
bool Apsis::Agent::Movers::GridlockDown::update(float elapsed,
  ...
  if (states.count(Apsis::State::MOVING_LEFT)  +
      ...
      states.count(Apsis::State::MOVING_UP) > 0) {
    return false;
  }

  if (_inputEngine->isEventHeld(Apsis::Action::PLAYER_1_DOWN)) {
    if (states.count(Apsis::State::MOVING_DOWN) == 0) {
      states.insert(Apsis::State::MOVING_DOWN);
    }
    updated.y += elapsed * 96.0f;

    return true;
  }
  else if (states.count(Apsis::State::MOVING_DOWN) != 0) {
    updated.y += elapsed * 96.0f;
    ...
    if (from_grid_1 > from_grid_0) {
      ...
      states.erase(Apsis::State::MOVING_DOWN);
    }
...
char* Apsis::Agent::Movers::GridlockDown::rule() {
  return "can move down locked to a grid";
}