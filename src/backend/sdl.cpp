...
bool Apsis::Backend::Sdl::poll(Apsis::Event& event) {
#ifndef NO_SDL
  SDL_Event sdl_event;

  if (SDL_PollEvent(&sdl_event)) {
    if(sdl_event.type == SDL_QUIT) {
      event.type = Apsis::Event::Type::Quit;
      return true;
    }

    event.pressed = _translateSDLKey(&event.binding, &sdl_event);
    event.type = Apsis::Event::Type::Input;

    return event.binding.key != Apsis::Key::NONE;
  }
#endif
  return false;
}
